...
choice = random.randint(0,len(fn_c0)-1)
...
case = (y_true==2)*(y_pred==0)
index = np.argwhere(case==1)
choice = random.randint(0,len(index)-1)
sample = int(index[choice])
print("Label of sampled image:",y_true[sample])
print("Prediction for sampled image:",y_pred[sample])
print("Name of sampled image:",input_names[sample])
img = Image.fromarray(inputs[int(index[0]),], mode = 'RGB')
display(img)
